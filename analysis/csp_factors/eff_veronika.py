...
def epsmKK(input_file1, input_file2, input_tree_name_file1, input_tree_name_file2, output_dir, mode, year):
    ...
    for i in range(len(mkk_bins)):
        ...
        #     print(f"tu puta madre: {mkk_histos[-1].GetBinContent(j)} -- {hb[i][1][j]}" )
    ...
    for j in range(len(hb)):
        ...
        if(j == 0 or j == 5):
            ...
            for i in range(NBINS):
                ...
                if j != 0 and hb[j][1][i] < mLL and SWAVE:
                    ratio = 0.
    ...
    for j in range(len(mkk_histos)):
        if(j == 0 or j == 5):
            ...
            for i in range(NBINS):
                ...
                masses[j].append(hmkk_WIDE.GetBinCenter(i+1))
        else:
            NBINS = NBINS_NARROW
            # print("NBINS NARROW =",NBINS_NARROW)
            for i in range(NBINS):
                ...
                ratios[j].append(ratio)
                masses[j].append(hmkk_NARROW.GetBinCenter(i+1))  
...
if __name__ == '__main__':
    ...
    input_file2 = "/scratch46/marcos.romero/MC_Csp_standalone/BsMuMuKK_2015.root"
    input_tree_name_file1 = "DecayTree"
    input_tree_name_file2 = "bsmumukk"

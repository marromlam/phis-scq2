...
if __name__ == "__main__":
  ...
  if mode == 'Bd2JpsiKstar':
    ...
    wcorr = ['sWeight*kinWeight', 'sWeight']
  ...
  xncorr, yncorr = histogram.compare_hist(data = [tdf[branch], odf[branch]],
                                    weights=[tdf.eval(ncorr[1]), odf.eval(ncorr[0])],
                                    bins=60, range=trang,
                                    density=True)
  xwcorr, ywcorr = histogram.compare_hist(data = [tdf[branch], odf[branch]],
                                    weights=[tdf.eval(wcorr[1]), odf.eval(wcorr[0])],
                                    bins=60, range=orang,
                                    density=True)

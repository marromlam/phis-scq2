...
def expected_value(angacc, cosK, cosL, hphi, project=None):
  eff = ristra.zeros_like(cosK)
  try:
    _angacc = ristra.allocate(np.array(angacc))
  except:
...
for var in ['cosK', 'cosL', 'hphi']:
  ...
  for year in [2015,2016]:#,2017,2018]:
    for trigger in ['biased', 'unbiased']:
      ...
      angacc = Parameters.load(f'output/params/angular_acceptance/{year}/Bs2JpsiPhi/v0r1_naive_{trigger}.json')
      ...
      for i in range(1,4):
        angacc = Parameters.load(f'output/params/angular_acceptance/{year}/Bs2JpsiPhi/v0r1_naive2knots{i}_{trigger}.json')
        eff = angeff_plot(angacc, cosKd, cosLd, hphid, proj)
        eff /= np.trapz(eff, x)
      ...
      fig.savefig(f'output/figures/angular_acceptance/{year}/Bs2JpsiPhi/v0r1_naive2knots{var.title()}_{trigger}.pdf')

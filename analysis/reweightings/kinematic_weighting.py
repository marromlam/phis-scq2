...
def kinematic_weighting(
    ...
    for trig in TRIGGER:
        if config.user["reweightings_per_trigger"]:
            ...
            ctdf = tdf
        cweight = reweight(
            codf.get(original_vars),
            ctdf.get(target_vars),
            codf.eval(original_weight),
            ctdf.eval(target_weight),
            n_estimators,
            learning_rate,
            max_depth,
            min_samples_leaf,
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--original-treename", default="DecayTree", help="Name of the original tree"
    )
    p.add_argument("--target-file", help="File to reweight to")
    p.add_argument(
        "--target-treename", default="DecayTree", help="Name of the target tree"
    )
    p.add_argument(
        "--output-file", help="Branches string expression to calc the weight."
    )
    p.add_argument(
        "--weight-set",
        default="kbsWeight",

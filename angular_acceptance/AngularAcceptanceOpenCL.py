...
context_cl = cl.create_some_context()  # Initialize the Context
queue = cl.CommandQueue(context_cl)  # Instantiate a Queue
...
vars_true_h = vars_true_h[:MAX_EVENTS,:MAX_EVENTS]
...
vars_true_d = pycl_array.to_device(queue, vars_true_h).astype(np.float64)
vars_reco_d = pycl_array.to_device(queue, vars_reco_h).astype(np.float64)
sweights_d  = pycl_array.to_device(queue, sweights_h).astype(np.float64)
...
pdf_true_d  = pycl_array.to_device(queue, pdf_true_h).astype(np.float64)
fk_true_d   = pycl_array.to_device(queue, fk_true_h).astype(np.float64)
pdf_reco_d  = pycl_array.to_device(queue, pdf_reco_h).astype(np.float64)
fk_reco_d   = pycl_array.to_device(queue, fk_true_h).astype(np.float64)
weights_d   = pycl_array.to_device(queue, weights_h).astype(np.float64)
...
kernel_path = '/home3/marcos.romero/phis-scq/opencl/DifferentialCrossRate.cl'
DifferentialCrossRate = cl.Program(context_cl,open(kernel_path,"r").read()).build()
clDiffRate  = DifferentialCrossRate.pyDiffRate
...
def getCrossRate_cu(vars,pdf,pars):
  """
...
def getCrossRate_cl(vars,pdf,pars):
  """
  Look at kernel definition to see help
  """
  clDiffRate( queue, pdf.shape, None, vars.data, pdf.data,
              np.float64(pars["G"]), np.float64(pars["DG"]),
              np.float64(pars["DM"]), np.float64(pars["CSP"]),
              np.float64(pars["APlon"]), np.float64(pars["ASlon"]),
              np.float64(pars["APpar"]), np.float64(pars["APper"]),
...
t0 = timer()
foo = getCrossRate_cu(gpuarray.to_gpu(vars_true_h).astype(np.float64),
                      gpuarray.to_gpu(pdf_true_h).astype(np.float64),
                      subs_dict)
t_cuda = timer()-t0

t0 = timer()
bar = getCrossRate_cl(pycl_array.to_device(queue, vars_true_h).astype(np.float64),
                      pycl_array.to_device(queue, pdf_true_h).astype(np.float64),
                      subs_dict)
t_opencl = timer()-t0

print(foo)
print(bar)
print(t_cuda,t_opencl)
BREAK

...
def pdf_reweighting(mcsample, mcparams, rdparams):
  print(mcparams.valuesdict())
  badjanak.delta_gamma5_mc(mcsample.true, mcsample.pdf, use_fk=1, set_tagging=2,
                           **mcparams.valuesdict(), tLL=tLL, tUL=tUL)
  original_pdf_h = mcsample.pdf.get()
  badjanak.delta_gamma5_mc(mcsample.true, mcsample.pdf, use_fk=0, set_tagging=2,
                           **mcparams.valuesdict(), tLL=tLL, tUL=tUL)
  original_pdf_h /= mcsample.pdf.get()
  badjanak.delta_gamma5_mc(mcsample.true, mcsample.pdf, use_fk=1, set_tagging=2,
    ...
    for dt in dy.values():
      badjanak.delta_gamma5_data(dt.data, dt.lkhd, **pars_dict,
                  ...
                  **dt.angacc.valuesdict(),
                  ...
                  tLL=tLL, tUL=tUL, use_timeacc=0, use_angacc=1,
                  use_timeres=0, set_tagging=2)  #set_tagging = 0
...
def do_fit(verbose=True):
  ...
  params = Parameters.build(result.params, names)
  print('Correlation')
  #print(params.corr(['fPlon', 'fPper', 'dPper', 'dPpar', 'DGsd']))
  print(params.corr(['fPlon', 'fPper', 'dPpar']))
  #exit()
...
def do_mc_combination(verbose):
  ...
  for y, dy in mc.items(): # loop over years
    for trigger in ['biased','unbiased']:
      ...
      data[y][trigger].angacc = Parameters.clone(std)
      data[y][trigger].angaccs[i] = Parameters.clone(std)
...
if __name__ == '__main__':
  ...
  real += ['mHH','0*sigmat', 'genidB','genidB', '0*time', '0*time']
  ...
  mass = [badjanak.config['mHH'][0], badjanak.config['mHH'][-1]]
  data = {}
  ...
  for i, y in enumerate(YEARS):
    ...
    for t, coeffs in zip(['biased','unbiased'],[coeffs_biased,coeffs_unbiased]):
      ...
      if bkgcat==True:
        data[y][t].chop(cuts_and(trigger_scissors(t), CUT,'bkgcatB != 60.0', '(evtN % 2) != 0', 'log(BDTFchi2) >=0', 'logIPchi2B >= 0'))#, 'log(BDTFchi2) >=0'))
        print(data[y][t])
      else:
  ...
  pars.add(dict(name="dPper", value=3.07,
            free=False, latex=r"\delta_{\perp} - \delta_0"))
  ...
  pars.add(dict(name="DGsd", value= -0.01597,
  ...
  if ok:
    ...
    for key in names:
      ...
      x = np.insert(x,0,-0.1)
      x = np.append(x, len(params_forPlotting)-0.90)

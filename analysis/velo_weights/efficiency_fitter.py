from ipanema import (Sample, Parameters, ristra, plotting, histogram, optimize)
import argparse
import numpy as np
...
if __name__ == '__main__':
  ...
  x = np.array(x); ux_low = np.array(ux_low); ux_upp = np.array(ux_upp)
  ux = np.array(ux_low)
  y = np.array([eff[i].n for i in eff.keys()])
  uy = np.array([eff[i].s for i in eff.keys()])

  # fcn
  model = lambda x, a, c: a * (1 + c * x**2)
  ...
  res = optimize(fcn, pars, method='minuit', fcn_kwgs={"x":x, "y":y, "uy":uy})#,
  ...
  axplot.plot(_x, _y)
  axpull.fill_between(x, histogram.pull_pdf(_x, _y, x, y, ux, uy),

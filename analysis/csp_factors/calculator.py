...
def Blatt_Weisskopf_VC( q, L = 1, isB = 0):
    ...
    if (L==1): 
        return Sqrt(1/(1+z))
    if (L==2): 
        return Sqrt((1./ ((z-3)*(z-3) + 9*z)))
                           
    return Sqrt(1./(Pow(z*z -45*z+105,2) +25*z*(2*z-21)*(2*z-21)))
...
Bs2JpsiKK_ps_S = Sqrt(p*q)*p*Bs2f0Jpsi_BW
Bs2JpsiKK_ps_P = Sqrt(p*q)*q*phi2KK_BW
...
Bs2JpsiKK_ps_S = simplify(Bs2JpsiKK_ps_S.subs(Mdau2,MKp))
Bs2JpsiKK_ps_P = simplify(Bs2JpsiKK_ps_P.subs(Mdau2,MKp))
...
def calculate_Csp_factors(input_dir1, input_dir2, year, output):
...
if __name__ == '__main__':
    ...
    calculate_Csp_factors(**vars(args))

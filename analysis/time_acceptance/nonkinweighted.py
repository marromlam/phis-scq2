...
def argument_parser():
  ...
  parser.add_argument('--BdMC-sample',
    default = '/scratch17/marcos.romero/phis_samples/v0r2/2016/MC_Bd2JpsiKstar/test.root',
    help='Bd2JpsiKstar MC sample')
  parser.add_argument('--BdDT-sample',
    default = '/scratch17/marcos.romero/phis_samples/v0r2/2016/Bd2JpsiKstar/test.root',
    help='Bd2JpsiKstar data sample')
  # Output parameters
  parser.add_argument('--BsMC-params',
    default = 'output/time_acceptance/parameters/nonkinweighted/2016/MC_Bs2JpsiPhi_dG0/test_biased.json',
    help='Bs2JpsiPhi MC sample')
  parser.add_argument('--BdMC-params',
    default = 'output/time_acceptance/parameters/nonkinweighted/2016/MC_Bs_Bd_ratio/test_biased.json',
    help='Bd2JpsiKstar MC sample')
  parser.add_argument('--BdDT-params',
    default = 'output/time_acceptance/parameters/nonkinweighted/2016/Bs2JpsiPhi/test_biased.json',
    help='Bd2JpsiKstar data sample')
  # Configuration file ---------------------------------------------------------
  parser.add_argument('--mode',
    default = 'nonkinweighted',
    help='Configuration')
  ...
  parser.add_argument('--flag',
    default = 'test',
    help='Flag identifying the sample (the actual filename)')
  parser.add_argument('--trigger',
    default = 'biased',
    help='Trigger(s) to fit [comb/(biased)/unbiased]')
  # Report
  parser.add_argument('--pycode',
    default = 'time_acceptance/nonkinweighted.py',
...
NAME = os.path.splitext(os.path.basename(args['pycode']))[0]
FLAG = args['flag']
YEAR = args['year']
...
def plot_spline(params, time, weights, conf_level=1, bins=30,
                ylabel=None, legend = None, color=None,
                fig= None, axplot=None):
  """
  Hi Marcos,
  ...
  axplot.plot(x,y_nom/y_spl, color=color, label=legend)
  ...
  if conf_level != 0:
    axplot.fill_between(x, y_upp/y_spl, y_low/y_spl, alpha=0.2,
                        facecolor=color,edgecolor="none",
                        label='$'+str(conf_level)+'\sigma$ confidence band')
  axplot.set_xlabel(r'$t$ [ps]')
  axplot.set_ylabel(r'%s [a.u.]' % ylabel)
  return fig, axplot
...
# Dumping parameters results
#    The following loop is writting parameters in a json file to be used by
#    next steps of the pipeline, and tex, to be inserted in latex documents.
com_flag = f'{FLAG}_{trigger}_nonkinweighted'
for name, cat in zip(cats.keys(),cats.values()):
  ...
  cat.params.dump(os.path.join(cat.pars_path,f'{com_flag}'))
  # latex export
  with open(os.path.join(cat.pars_path,f'{com_flag}.tex'), "w") as text:
    text.write(cat.params.dump_latex())
  print( os.path.join(cat.pars_path,f'{com_flag}') )

# Get baseline parameters
#    The spline plot will include the baseline fit + this nonkinweighted fit
#    results. In this way, it's easy to compare the effect of the kinweights
#    in the acceptance.
base_pars = {}
for name, cat in zip(cats.keys(),cats.values()):
  base_pars[name] = Parameters.load(
    os.path.join(cat.pars_path.replace('nonkinweighted/',''),
                 f'{FLAG}_{trigger}.json')
    )
...
for name, cat in zip(cats.keys(),cats.values()):
  plot_fcn_spline(
    ...
    cat.time.get(),
    cat.weight.get(),
    name = os.path.join(cat.figs_path,f'{com_flag}_fit_log.pdf'),
    log=True
  )
print(f"Plotted {os.path.join(cat.figs_path,f'{com_flag}_fit_log.pdf')}")
...
for name, cat in zip(cats.keys(),cats.values()):
  plot_fcn_spline(
    ...
    cat.time.get(),
    cat.weight.get(),
    name = os.path.join(cat.figs_path,f'{com_flag}_fit.pdf'),
    log=False
  )
print(f"Plotted {os.path.join(cat.figs_path,f'{com_flag}_fit.pdf')}")
...
plot_spline(# BsMC
  ...
  ylabel=r'$\varepsilon_{\mathrm{MC}}^{B_s^0}$',
  conf_level=1,
  legend='baseline', color='C0',
  bins=25, fig=fig, axplot=axplot
)
plot_spline(# BsMC
  cats['BsMC'].params,
  cats['BsMC'].time.get(),
  cats['BsMC'].weight.get(),
  ylabel=r'$\varepsilon_{\mathrm{MC}}^{B_s^0}$',
  conf_level=0,  color='C2',
  legend='without kinematic weights',
  bins=25, fig=fig, axplot=axplot
)
axplot.legend()
fig.savefig(os.path.join(cats['BsMC'].figs_path,f'{com_flag}_spline.pdf'))

# ratio
fig, axplot = plotting.axes_plot()
plot_spline(# BsMC
  base_pars['BsMC']+base_pars['BdMC'],
  cats['BdMC'].time.get(),
  cats['BdMC'].weight.get(),
  conf_level=1, legend='baseline',  color='C0',
  bins=25, fig=fig, axplot=axplot
)
plot_spline(# BsMC
  ...
  cats['BdMC'].time.get(),
  cats['BdMC'].weight.get(),
  ylabel=r'$\varepsilon_{\mathrm{MC}}^{B_d}/\varepsilon_{\mathrm{MC}}^{B_s^0}$',
  conf_level=0, legend='without kinematic weights',  color='C2',
  bins=25, fig=fig, axplot=axplot
)
axplot.legend()
fig.savefig(os.path.join(cats['BdMC'].figs_path,f'{com_flag}_spline.pdf'))

# BsDT
fig, axplot = plotting.axes_plot()
plot_spline(# BsMC
  base_pars['BsMC']+base_pars['BdMC']+base_pars['BdDT'],
  cats['BdDT'].time.get(),
...
plot_spline(# BsMC
  ...
  cats['BdDT'].time.get(),
  cats['BdDT'].weight.get(),
  ylabel=r'$\varepsilon_{\mathrm{data}}^{B_s^0}$',
  conf_level=0, legend='without kinematic weights', color='C2',
  bins=25, fig=fig, axplot=axplot
)
axplot.legend()
fig.savefig(os.path.join(cats['BdDT'].figs_path,f'{com_flag}_spline.pdf'))

...
from ROOT import AddressOf, addressof
...
def argument_parser():
    ...
    parser.add_argument('--input-tree-name',
                        default='DecayTree', help='Name of the input tree')
    parser.add_argument(
        '--input-branch', help='Name of the branch to be fitted')
    parser.add_argument('--output-file', help='Output ROOT file')
    parser.add_argument(
        '--plot-dir', help='Location in which to save the plots')
...
def fit_branch(input_file, input_tree_name, input_branch, output_file, plot_dir, mode, year):
    ...
    # Bs mass info
    mean = RooRealVar("bs #mu", "mean of bs", mean_initial,
                      lower_cut, upper_cut, "")
    ...
    gauss = RooAddModel("gauss", "gauss", RooArgList(
        gauss1, gauss2), RooArgList(frac))
    # bkg flat
    slope_bs = RooRealVar("slope bs", "slope bs", -0.01, -2.0, 2.0)
    bs_bkg = RooExponential("bs_bkg", "bs bkg", bs_mass, slope_bs)
    # number of events
    nsig = RooRealVar("nsig", "number of signal events",
                      s_fit[0], s_fit[1], s_fit[2], "")
    nbkg = RooRealVar("nbkg", "number of backgnd events",
                      b_fit[0], b_fit[1], b_fit[2], "")
    # signal bkg pdfs
    model = RooAddPdf("bs pdf", "bs pdf", RooArgList(
        gauss, bs_bkg), RooArgList(nsig, nbkg))

    n_CPUs = multiprocessing.cpu_count()

    fitresult = model.fitTo(data, RooFit.Save(kTRUE), RooFit.Minos(
        kFALSE), RooFit.NumCPU(max(n_CPUs-2, 1)))

    frame1 = bs_mass.frame(RooFit.Title("Bs Mass"), RooFit.Bins(
        140), RooFit.Range(lower_cut, upper_cut))
    frame2 = bs_mass.frame(RooFit.Bins(140), RooFit.Title(""))

    data.plotOn(frame1, RooFit.Name("dataHist"), RooFit.MarkerSize(
        0.8), RooFit.DataError(RooAbsData.SumW2))
    model.plotOn(frame1, RooFit.LineColor(kBlue))
    model.plotOn(frame1, RooFit.Components("gauss"), RooFit.LineColor(6))
    model.plotOn(frame1, RooFit.Components("bs_bkg"),
                 RooFit.LineColor(3), RooFit.LineStyle(4))

    data.plotOn(frame2, RooFit.Name("dataHist"), RooFit.MarkerSize(
        0.8), RooFit.DataError(RooAbsData.SumW2))
    model.plotOn(frame2, RooFit.LineColor(kBlue))

    name_fit = "#splitline{#splitline{#splitline{N_{sig} = " + str(round(nsig.getVal())) + " #pm " + str(round(nsig.getError())) + "}{#mu = " + str(round(mean.getVal(), 2)) + " #pm " + str(round(mean.getError(), 2)) + " MeV/c^{2}}}{#splitline{#sigma_{1} = " + str(round(sigma1.getVal(), 2)) + " #pm " + str(round(sigma1.getError(
    ), 2)) + "MeV/c^{2}}{#sigma_{2} = " + str(round(sigma2.getVal(), 2)) + " #pm " + str(round(sigma2.getError(), 2)) + " MeV/c^{2}}}}{S/B = " + str(round(nsig.getVal()/nbkg.getVal(), 3)) + " #pm " + str(round(TMath.Sqrt(pow(nsig.getError()/nbkg.getVal(), 2) + pow(nsig.getVal()/pow(nbkg.getVal(), 2) * nbkg.getError(), 2)), 3)) + "}"
    ...
    sData = RooStats.SPlot("sData", "An SPlot", data,
                           model, RooArgList(nsig, nbkg))

    print("Check SWeights:")
    print("Yield of signal is", nsig.getVal(),
    ...
    sWBranch = new_tree.Branch('sw', addressof(
        sWeight_struct, 'sWValue'), 'sw/D')

...
WITHIN_KERNEL
ftype mygauss(const ftype x, const ftype mu, const ftype sigma, const ftype xLL, const ftype xUL)
{
            ...
            const ftype xLL, const ftype xUL)
{
  ...
  const ftype num = exp((rpow(sigma,2) + 2*mu*tau - 2*tau*x)/(2.*rpow(tau,2)))*sqrt(M_PI/2.)*sigma*erfc((rpow(sigma,2) + tau*(mu - x))/(sqrt(2.)*sigma*tau));
  const ftype den = sqrt(Pi/2.)*sigma*tau*(-1 + erf((-mu + xUL)/(sqrt(2.)*sigma)) + erfc((-mu + xLL)/(sqrt(2.)*sigma)) + 
     exp((pow(sigma,2) + 2*tau*(mu - xLL - xUL))/(2.*pow(tau,2)))*(exp(xUL/tau)*erfc((pow(sigma,2) + mu*tau - tau*xLL)/(sqrt(2.)*sigma*tau)) - 
        exp(xLL/tau)*erfc((pow(sigma,2) + mu*tau - tau*xUL)/(sqrt(2.)*sigma*tau))));
  return num/den;
}
...
WITHIN_KERNEL
ftype time_fit(
  const ftype time, GLOBAL_MEM const ftype *fres,
...
)
{
  ...
  for (int i=0; i<2; ++i)
  {
    ...
    long_live += (1-fsl) * fres[i] * myexp(time, taul, mu, sigma[i], tLL, tUL);
  }
...
{
    const int idx = get_global_id(0);
    prob[idx] = time_fit(time[idx], fres, mu, sigma, fprompt, fll, fsl, fwpv, taul, taus,
                         tau1, tau2, share, tLL, tUL);
}

...
# Get peilian
with open('/home3/marcos.romero/log_events_BdMC.out') as fp:
#with open('/home3/marcos.romero/log_v0r0_BdMC.out') as fp:
#with open('/home3/marcos.romero/log_v0r0_BdMC_hdfitter.out') as fp:
  crap = fp.readlines()


crap = crap[5:-1] # remove crap from crap
#crap = crap[2:] # remove crap from crap
peilian = []
for shit in crap:
  peilian.append([np.float64(i) for i in shit.split(' *')[1:-1]])
  #peilian.append([np.float64(i) for i in shit.replace(10*' ',',').replace(9*' ',',').replace(8*' ',',').replace(7*' ',',').replace(6*' ',',').replace(5*' ',',').replace(4*' ',',').replace(3*' ',',').replace(2*' ',',').replace(1*' ',',').replace('\n','').split(',')[1:]])
peilian = np.array(peilian)
peilian


# Get marcos
marcos  = uproot.open('/scratch17/marcos.romero/phis_samples/2015/MC_Bd2JpsiKstar/v0r5.root')['DecayTree'].arrays(['time','sw','polWeight','pdfWeight','kinWeight'])
marcos = np.vstack(list(marcos.values())).T

# Get simon
simon = uproot.open('/scratch03/marcos.romero/phisRun2/UNTOUCHED_SIMON_SIDECAR/BdJpsiKstar_MC_2015_UpDown_CombDSTLDSTMDST_20181101_CombSim09bSim09c_tmva_cut58_sel_sw_trigCat_PDFWeightsSetRun1Bd.root')['PDFWeights'].array('PDFWeight')
...
plt.plot(marcos[:,0]-peilian[:,0])
plt.plot(marcos[:,1]-peilian[:,1])
plt.plot(marcos[:,2]-peilian[:,3])
plt.plot(marcos[:,3]-peilian[:,2])

np.amax( (marcos[:,3]-peilian[:,2]) / (marcos[:,3]) )
np.amax( (marcos[:,3]-simon[:]) / (marcos[:,3]) )

plt.plot(marcos[:,3]-simon[:])

log_v0r0_BdMC.out
peilian[:,4]

plt.plot(marcos[:,3]-simon[:],label='me-simon')
plt.plot(marcos[:,3]-peilian[:,3],label='peilian-simon')
plt.legend()
plt.title('pdfWeight v0r0')
...
def likelihood_s1xs2xs3xerf(params, data, weight=False, prob=None):
  pars = params.valuesdict()
  if not prob:                                             # for ploting, mainly
    samples = list( map(ristra.allocate,data) )
    prob = list( map(np.zeros_like,samples) )
    bsjpsikk.s1xs2xs3xerf(*samples, *prob, **pars)
    return [ p.get() for p in prob ]
  else:                               # Optimizer.optimize ready-to-use function
    bsjpsikk.s1xs2xs3xerf(*data, *prob, **pars)
    if weight:
      result  = np.concatenate(( (ristra.log(prob[0])*weight[0]).get(),
                                 (ristra.log(prob[1])*weight[1]).get(),

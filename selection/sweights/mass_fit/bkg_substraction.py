...
with open('selection/sweights/branches.yml') as file:
...
def mass_fitter(odf, mass_range=False, mass_branch="B_ConstJpsi_M_1",
    ...
    # print(rd.df.shape[0])
    rd.chop(current_cut)
    # rd.df[f"{mass_weight}"] = rd.df[f"{mass_weight}"]*np.sum(rd.df[f"{mass_weight}"])/np.sum(rd.df.eval(f"{mass_weight}*{mass_weight}"))
    # print(mass_weight)
    # exit()
    # print(rd.df.shape[0])
    if verbose:
    ...
    if model == "dgauss":
      ...
      print("We are not prepared for this model")
...
if __name__ == '__main__':
    ...
    # branches = CONFIG['all_branches'][args['mode']]
    # _branches = sum([get_vars_from_string(k) for k in branches.values()],[])
    branches = [get_vars_from_string(v) for v in list(CONFIG['all_branches'][args['mode']].values())]
    branches = sum(branches, []) # flatten
    branches = list(dict.fromkeys(branches)) # remove duplicated ones

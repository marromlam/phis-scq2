...
if __name__ == "__main__":
  ...
  noe = (order_hphi+1)*(order_cosL+1)*(order_cosK+1)    # number of coefficients
  ...
  for k, var in enumerate(['cosK', 'cosL', 'hphi']):
    ...
    print(df.shape)
    histdd = np.histogramdd(df[['cosK','cosL','hphi']].values,
                            bins=(nob,nob,nob),
    ...
    _umc = np.sqrt(_mc); _uth = 0*np.sqrt(_th)
    ...
    eff = badjanak.analytical_angular_efficiency(
                np.array([x.n for x in upeff]), cosKd, cosLd, hphid,
                None, order_cosK, order_cosL, order_hphi)
    eff = np.sum(eff*th, dir) / np.sum(th, dir)
    norm = 1/(np.trapz(y, x)/np.trapz(eff, xh))
    scale = np.max(y)
    
    # compute confidence bands
    yunc = uncertainty_wrapper(lambda p:
              np.mean(th*badjanak.analytical_angular_efficiency(p, cosKd, cosLd, hphid, None,
    ...
    # plot
    ax.errorbar(bounds*x, y/scale, yerr=uy/scale, xerr=bounds*ux,
                fmt='.', color='C0')
    ax.plot(bounds*xh, eff/norm/scale,
            label='full range', color='C0')
    ax.fill_between(bounds*x, yh/norm/scale, yl/norm/scale,
                    color='C0', alpha=0.2)
    ...
    if len(angaccs) > 1:
      for i, angacc_ in enumerate(angaccs[1:]):
        ...
        print(df.shape)
        histdd = np.histogramdd(df[['cosK','cosL','hphi']].values, bins=(nob,nob,nob),
                                weights=df.eval(f'polWeight*sw/gb_weights'),
                                range=[(-1,1),(-1,1),(-np.pi,np.pi)])
        mccounts, (ecosK, ecosL, ehphi) = histdd
        ...
        # create arrays 
        mc = np.float64([data_3d(i) for i in range(nob**3)]).reshape(nob,nob,nob)
        th = np.float64([prediction_3d(i) for i in range(nob**3)]).reshape(nob,nob,nob)
        ...
        print(upeff)
        upeff = [p.uvalue for p in upeff.values()]
        eff = badjanak.analytical_angular_efficiency(np.array([x.n for x in upeff]), cosKd, cosLd, hphid,
                               None, order_cosK, order_cosL, order_hphi)
        eff = np.sum(eff*th, dir) / np.sum(th, dir)
        norm = 1/(np.trapz(y, x)/np.trapz(eff, xh))
        #scale = np.max(y)
        ...
        ax.errorbar(bounds*x, y/scale, yerr=uy/scale, xerr=bounds*ux,
                    fmt='.', color=f'C{i+1}')
        ax.plot(bounds*xh, eff/norm/scale,
                label=f'$t \in ({ll},{ul})$', color=f'C{i+1}')
        ax.fill_between(bounds*x, yh/norm/scale, yl/norm/scale,

...
try:
  pars = Parameters.load(f"output/params/physics_params/run2/Bs2JpsiPhi/{VERSION}_run2_run2Dual_vgc_amsrd_{TIMEACC}_amsrd_{TRIGGER}.json").valuesdict(False)
except:
  pars = Parameters.load(f"output/params/physics_params/run2/Bs2JpsiPhi/{VERSION}_run2_run2Dual_vgc_amsrd_{TIMEACC}_amsrd_unbiased.json").valuesdict(False)
...
all_samples = [f"/scratch46/marcos.romero/sidecar14/{y}/Bs2JpsiPhi/{VERSION}.root" for y in YEARS]
all_csp = [ f"output/params/csp_factors/{y}/Bs2JpsiPhi/{VERSION}_vgc.json" for y in YEARS]
all_flavor = [ "output/params/flavor_tagging/2016/Bs2JpsiPhi/v0r5_amsrd.json" for y in YEARS]
all_timeacc = {
...
def pdf_projector(params, edges, var='time', timeacc=False, angacc=False,
  ...
  # acc /= np.trapz(acc, _x)
  acc = 1
  _pdf = prob(params, cosKLL, cosKUL, cosLLL, cosLUL, hphiLL, hphiUL, timeLL, timeUL, avgres=avgres)
...
for km, vm in samples.items():
  for ky, vy in vm.items():
    for kt, vt in vy.items():
      _hvar = hist(vt.df[branch].values, bins=edges, weights=vt.df[wvar].values)
      ...
      hyerr.append([np.nan_to_num(_hvar.errl), np.nan_to_num(_hvar.errh)])

...
config_file = hjson.load(open(PATH+'/config/'+args['config']+'.json'))

# Paths in phis-scq
from tools.paths import TIMEACC_PATH, SAMPLES_PATH
...
def lkhd_ratio_spline(parameters, data, weight = None, prob = None):
  ...
  if not prob:                                             # for ploting, mainly
    ...
    ratio_spline_time_acceptance(samples, prob, pars_dict)
    return [ p.get() for p in prob ]
  else:                               # Optimizer.optimize ready-to-use function
    ratio_spline_time_acceptance(data, prob, pars_dict)
    if weight is not None:
...
# Select samples
samples = {}
samples['BsMC'] = os.path.join(SAMPLES_PATH,args['BsMC_sample'])
samples['BdMC'] = os.path.join(SAMPLES_PATH,args['BdMC_sample'])
samples['BdDT'] = os.path.join(SAMPLES_PATH,args['BdDT_sample'])
...
for name, sample in zip(samples.keys(),samples.values()):
  ...
  for trig, bool in zip(triggers.keys(),triggers.values()):
    ...
    param_path  = PATH+'/input/params-'
    param_path += os.path.splitext(os.path.basename(sample))[0][:-5]
    param_path += '-baseline.json'
...
if FIT_RATIO:
  for trig in ['_unbiased', '_biased']:
    ...
    print('Fitting %s category...' % name)
    fits[name] = optimize(lkhd_ratio_spline, method="bfgs",
...
if FIT_FULL:
  for trig in ['_unbiased', '_biased']:
    ...
    fits[name] = optimize(lkhd_full_spline, method="hesse",
                    params=cats['BsMC'+trig].params+cats['BdMC'+trig].params+cats['BdDT'+trig].params,
                    kwgs={'data':  [cats['MC_Bs2JpsiPhi'+trig].time,
                                   cats['MC_Bd2JpsiKstar'+trig].time,
                                   cats['Bd2JpsiKstar'+trig].time],
                        'prob':   [cats['MC_Bs2JpsiPhi'+trig].lkhd,
                                   cats['MC_Bd2JpsiKstar'+trig].lkhd,
                                   cats['Bd2JpsiKstar'+trig].lkhd],
                        'weight': [cats['MC_Bs2JpsiPhi'+trig].weight,
                                   cats['MC_Bd2JpsiKstar'+trig].weight,
                                   cats['Bd2JpsiKstar'+trig].weight]},
                    verbose=True);
    fits[name].params.dump(TIMEACC_PATH+'/parameters/'+name)
    print('\n')

...
double getF(double cosK, double cosL, double hphi, int k)
{
  ...
  double sinphi = sin(hphi);
  double cosphi = cos(hphi);
                    ...
                    double G, double DG, double DM, double ti, double tf)
{
  ...
  double itb = (2*(DG*cosh(.5*DG*ti) + 2*G*sinh(.5*DG*ti))*exp(G*tf) - 2*(DG*cosh(.5*DG*tf) + 2*G*sinh(.5*DG*tf))*exp(G*ti))*exp(-G*(ti + tf))/(-pow(DG, 2) + 4*pow(G, 2));
  double itc = ((-DM*sin(DM*ti) + G*cos(DM*ti))*exp(G*tf) + (DM*sin(DM*tf) - G*cos(DM*tf))*exp(G*ti))*exp(-G*(ti + tf))/(pow(DM, 2) + pow(G, 2));
  double itd = ((DM*cos(DM*ti) + G*sin(DM*ti))*exp(G*tf) - (DM*cos(DM*tf) + G*sin(DM*tf))*exp(G*ti))*exp(-G*(ti + tf))/(pow(DM, 2) + pow(G, 2));;

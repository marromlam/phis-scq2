...
for i, y in enumerate(years):
  ...
  data[f'{y}']['biased'].ang_weights = np.float64([1,1.02619,1.02592,-0.000808072,0.000768617,0.00020551,1.00647,0.000455222,0.000146616,-0.000731596])
  ...
  data[f'{y}']['unbiased'].ang_weights = np.float64([1,1.0204196,1.0205028,0.0026313506,0.0031254275,-0.00032937306,1.0115991,0.00025576617,4.6120163e-06,-0.0013316976])
...
Parameter(name='fSlon1',          value=SWAVE*0.1**1,         min=SWAVE*0.00,   max=0.80,   free=SWAVE),
Parameter(name='fSlon2',          value=SWAVE*0.1**1,         min=SWAVE*0.00,   max=0.80,   free=SWAVE),
Parameter(name='fSlon3',          value=SWAVE*0.1**1,         min=SWAVE*0.00,   max=0.80,   free=SWAVE),
Parameter(name='fSlon4',          value=SWAVE*0.1**1,         min=SWAVE*0.00,   max=0.80,   free=SWAVE),
Parameter(name='fSlon5',          value=SWAVE*0.1**1,          min=SWAVE*0.00,   max=0.80,   free=SWAVE),
Parameter(name='fSlon6',          value=SWAVE*0.1**1,         min=SWAVE*0.00,   max=0.80,   free=SWAVE),
#
...
Parameter(name="dPper",           value=3.10,                 min= 2.8,   max=3.2,    free=True),
#
...
Parameter(name="DGsd",            value= 0.00,                min=-0.1,   max= 0.1,   free=True),
Parameter(name="DM",              value=17.757,           min=17.0,   max=18.0,   free=True),
# CSP parameters
  ...
  for y, dy in data.items():
    for t, dt in dy.items():
      bsjpsikk.diff_cross_rate_full(dt.data, dt.lkhd,
                                  ...
                                  coeffs = dt.coeffs,
                                  **pars_dict)
  prob = ristra.concatenate([dt.lkhd for dy in data.values() for dt in dy.values()])
  if weight:
    weight = ristra.concatenate([dt.weight for dy in data.values() for dt in dy.values()])
...
data['2016']['biased'].df['pdf'] = data['2016']['biased'].lkhd.get()
data['2016']['unbiased'].df['pdf'] = data['2016']['unbiased'].lkhd.get()
...
for i in range(0,12):
  if i<6:
    b.append(np.array(data['2016']['biased'].df.query(f'X_M>={mass[i]} & X_M<{mass[i+1]}')['pdf']))
  else:
    b.append(np.array(data['2016']['unbiased'].df.query(f'X_M>={mass[i-6]} & X_M<{mass[i-6+1]}')['pdf']))
...
data['2016']['biased'].df['pdf'] = data['2016']['biased'].lkhd
...
#shit = optimize(fcn_data, params=pars, method='hesse', kwgs={"data": data} )

shit = optimizers.Optimizer(fcn_data, params=pars, fcn_kwgs={"data": data} )
shit.prepare_fit()
shit._configure_minuit_(pars, params=pars)
...
# give it a call!
crap = minuit(parseminuit, forced_parameters=shit.result.param_vary, **shit._configure_minuit_(pars), print_level=1)
crap.migrad()
crap.migrad()
crap.hesse()
